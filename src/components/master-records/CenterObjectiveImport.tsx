...
interface CenterObjectiveImportProps {
  onImport: (objectives: Omit<CenterObjective, 'id'>[]) => void;
  onClose: () => void;
}

const CenterObjectiveImport: React.FC<CenterObjectiveImportProps> = ({ onImport, onClose }) => {
  ...
  const validateField = (value: any, field: string, row: number): ImportError | null => {
    ...
    switch (field) {
      case 'code':
        if (!/^OBJ-CENTRO-\d{4}-\d{2}$/.test(value)) {
          return {
            row,
            field,
            message: 'Código inválido (formato: OBJ-CENTRO-YYYY-NN)',
            data: value,
          };
        }
        break;
  ...
  const downloadTemplate = () => {
    ...
    XLSX.writeFile(wb, 'plantilla_objetivos_centro.xlsx');
  };
    ...
    try {
      ...
      reader.onload = async (event) => {
        try {
          ...
          const objectivesToImport: Omit<CenterObjective, 'id'>[] = [];

          jsonData.forEach((row: any, index) => {
            const rowErrors: ImportError[] = [];
            
            // Validate required fields
            ['code', 'name', 'center', 'category'].forEach(field => {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl">
        <div className="flex items-center justify-between p-4 border-b">
          <h2 className="text-lg font-semibold">Importar Objetivos de Centro</h2>
        ...
        <div className="p-6">
          ...
          <div className="text-sm text-gray-500">
            ...
            <ul className="list-disc pl-5 space-y-1">
              ...
              <li>Los campos code, name, center y category son obligatorios</li>
              <li>El código debe tener el formato OBJ-CENTRO-YYYY-NN</li>
...
export default CenterObjectiveImport;