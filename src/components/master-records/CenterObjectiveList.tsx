...
interface CenterObjectiveListProps {
  objectives: CenterObjective[];
  onEdit: (objective: CenterObjective) => void;
  onDelete: (id: string) => void;
  onToggleActive: (id: string) => void;
}

const CenterObjectiveList: React.FC<CenterObjectiveListProps> = ({ 
  objectives, 
  onEdit, 
  onDelete,
  onToggleActive 
}) => {
  const getPriorityColor = (priority: CenterObjective['priority']) => {
    switch (priority) {
      case 'high':
        ...
        <div key={objective.id} className="bg-white border rounded-lg shadow-sm hover:shadow transition-shadow">
          <div className="grid grid-cols-12 gap-4 p-4">
            ...
            <div className="col-span-4">
              ...
              <div className="text-sm text-gray-500 mt-1">{objective.center}</div>
            </div>
...
export default CenterObjectiveList;